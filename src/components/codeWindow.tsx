...
function CodeWindow({ files }: CodeWindowProps) {
  ...
  return (
    <div>
        ...
        {files.map(file => {
          ...
          return (
            <button
              key={file.filename}
              type="button"
              className={`dark:bg-stone-8 -mb-[1px] rounded-t-lg border border-slate-200 px-2 py-1 text-xs font-medium text-muted dark:border-stone-500
              ${
                isSelected
                  ? 'border-b-slate-100 bg-slate-100 text-primary dark:border-b-stone-800 dark:bg-stone-800 dark:text-onPrimary'
      ...
      {/* Code */}
      <pre
        className="overflow-x-auto rounded-lg rounded-tl-none border border-slate-200 bg-slate-100 p-2 text-sm text-primary dark:border-stone-500 dark:bg-stone-800 dark:text-onPrimary"
